...
def __Menu__(screen, screen_width, screen_height, LeftRectwidth, dt):
    ...
    if current_album_name != '':
        ...
        if len(tt_a[3]) > 18:
            tt_a[3] = tt_a[3][:15] + "..."
        text = font.render(tt_a[3], True, (245, 245, 245))
...
def play_music(music):
    ...
    if music is not None:
        if first_timee and current_album_name != music:
            if not pause_btn:
                print(f"Playing music: {music.split('/')[3]}")
        ...
        if os.path.exists(music):
            ...
            desired_component = path_components[3] if len(path_components) > 3 else ""
            ...
            for i in range(len(singles)):
                singles[i] = ForderFinder.get_music_file(singles[i])
            for i in range(len(albums)):
                albums[i] = ForderFinder.get_music_file(albums[i])
            ...
            music_folder = os.path.dirname(music)
            path_components = music.split('/')
            album_name = path_components[3] if len(path_components) > 3 else ""
