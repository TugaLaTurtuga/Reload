...
def display_folders(albums, singles, dt):
    ...
    if favourites != []:
        for favourite in favourites:
            if os.path.exists(favourite):
                ...
                try:
                    ...
                    btns_pos.append((LeftRectwidth / 2 - size / 2, y_offset))
                    btns_do.append(
                        lambda fav=favourite, path=album_path, comp=path_components[3]: ForderFinder.play_music(fav, comp, path,True))
        ...
        for album in albums:
            icon_path = os.path.join(album, 'icon.png')
            if not os.path.exists(icon_path):
                icon_path = 'Images/Volume/full_volume.png'
            icon = make_icon(pg.image.load(icon_path))
            screen.blit(icon, (LeftRectwidth / 2 - size / 2, y_offset))
            ...
            btns_pos.append((LeftRectwidth / 2 - size / 2, y_offset))
            btns_do.append(
                lambda music=album: ForderFinder.play_music(ForderFinder.get_music_file(os.path.join(music)), music.split('/')[3],
                                                            music, True))

            y_offset += reload_icon.get_height() + y_plus_offset

        for single in singles:
            icon_path = os.path.join(single, 'icon.png')
            if not os.path.exists(icon_path):
                icon_path = 'Images/Volume/full_volume.png'
            icon = make_icon(pg.image.load(icon_path))
            screen.blit(icon, (LeftRectwidth / 2 - size / 2, y_offset))
            ...
            btns_pos.append((LeftRectwidth / 2 - size / 2, y_offset))
            btns_do.append(
                lambda music=single: ForderFinder.play_music(ForderFinder.get_music_file(os.path.join(music)), music.split('/')[2],
                                                            music, True))
            y_offset += reload_icon.get_height() + y_plus_offset
